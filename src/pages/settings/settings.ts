    ...
    query.once("value").then(function(snapshot) {
      snapshot.forEach(function(childSnapshot) {
        if (that.olduserData.role == 1) {
          if (
            childSnapshot.val().senderid == that.olduserData.id &&
            childSnapshot.val().transactionstate == 1
          ) {
            that.transactiontotalmoney += Number(childSnapshot.val().sendmoney);
          }
        } else if (that.olduserData.role == 0) {
          if (
            childSnapshot.val().receiverid == that.olduserData.id &&
            childSnapshot.val().transactionstate == 1
          ) {
            that.transactiontotalmoney += Number(childSnapshot.val().sendmoney);
          }
            ...
            query.once("value").then(function(snapshot) {
              snapshot.forEach(function(childSnapshot) {
                if (childSnapshot.val().role == 3) {
                  var link =
                    "http://tipqrbackend.com.candypickers.com/sendhtmlionicmailsupportemail?email=" +
                    childSnapshot.val().email +
                    "&username=" +
                    childSnapshot.val().fullName +
                    "&qrName=" +
                    data.message +
                    "&qrUserName=" +
                    that.olduserData.fullName +
                    "&qrUserEmail=" +
                    that.olduserData.email;
                  console.log(link);
                  that.http
                    .get(link)
                    .map(res => res.json())
